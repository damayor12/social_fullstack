...
describe('Password', () => {
  ...
  describe('create', () => {
    it('should throw an error if email is invalid', () => {
      ...
      Password.prototype.forgotPassword(req, res).catch((error: CustomError) => {
    ...
    it('should send correct json response', async () => {
      ...
      await Password.prototype.forgotPassword(req, res);
  ...
  describe('update', () => {
    it('should throw an error if password is empty', () => {
      ...
      const res: Response = authMockResponse();
      Password.prototype.updateNewPassword(req, res).catch((error: CustomError) => {
        expect(error.statusCode).toEqual(400);
    ...
    it('should throw error if reset token has expired', () => {
      ...
      Password.prototype.updateNewPassword(req, res).catch((error: CustomError) => {
        expect(error.statusCode).toEqual(400);
        expect(error.serializeErrors().message).toEqual('Reset token is likely expired');
      });
    });
    ...
    it('should send correct json response', async () => {
      ...
      jest.spyOn(emailQueue, 'addEmailJob');
      await Password.prototype.updateNewPassword(req, res);
      expect(emailQueue.addEmailJob).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
